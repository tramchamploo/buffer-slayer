...
public class BatchJedisSetCommandsTest extends BatchJedisTestBase {
  ...
  @Test
  public void sadd() {
    long status = blocking(batchJedis.sadd("foo", "a"));
    assertEquals(1, status);

    status = blocking(batchJedis.sadd("foo", "a"));
    assertEquals(0, status);

    long bstatus = blocking(batchJedis.sadd(bfoo, ba));
    assertEquals(1, bstatus);

    bstatus = blocking(batchJedis.sadd(bfoo, ba));
  ...
  public void smembers() {
    ...
    Set<String> members = blocking(batchJedis.smembers("foo"));
    ...
    Set<byte[]> bmembers = blocking(batchJedis.smembers(bfoo));
