 ...
 */
public class FiberReporterTest {

  private FiberReporter<TestMessage, Integer> reporter;
  private InMemoryReporterMetrics metrics = InMemoryReporterMetrics.instance(ReporterMetricsExporter.NOOP_EXPORTER);
  ...
  public void flushIfExceedTimeout() throws InterruptedException {
    ...
    reporter = FiberReporter.fiberBuilder(sender)
        .timerThreads(1)
  ...
  public void flushThreadName() {
    ...
    reporter = FiberReporter.fiberBuilder(sender).build();
    reporter.report(newMessage(0));

    Iterator<Thread> iter = reporter.flushers.iterator();
    assertTrue(iter.hasNext());
    assertEquals("FiberReporter-0-flusher-0", iter.next().getName());
    ...
    assertTrue(iter.hasNext());
    assertEquals("FiberReporter-1-flusher-0", iter.next().getName());
  }
  ...
  public void lazyInitFlushThreads() throws InterruptedException {
    FakeSender sender = new FakeSender();
    reporter = FiberReporter.fiberBuilder(sender).flushThreads(1).build();
